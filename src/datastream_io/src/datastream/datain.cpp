...
extern bool extract_GNSSSolData_HKUSTGVINS(const char *bag_infilepath, const std::string &gnsssol_topic, std::list<datastreamio::RobotGVINS_GNSSSol> &gnsssol_datas)
{
    ...
    foreach (rosbag::MessageInstance const m, view)
    {
        if (m.instantiate<datastreamio::GVINS_GNSSPVTSolnMsg>() != nullptr)
        {
            // get gnss solution data
            datastreamio::GVINS_GNSSPVTSolnMsg::ConstPtr gnsssol_msg = m.instantiate<datastreamio::GVINS_GNSSPVTSolnMsg>();
            ...
            datastreamio::RobotGVINS_GNSSSol sol_data;
