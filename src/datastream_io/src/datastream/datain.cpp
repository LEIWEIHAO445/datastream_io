...
namespace dataio_common
{
     ...
     * @note        1. The imu data to read should be ros standard format
     ...
     * @note        1. The image data to read should be ros standard format
     *              2. The image data is saved as ros standard format defaultly
     *              3. If need, the GPS/Linux time will be converted
     *
     * @param[in]   char*      bag_infilepath      filepath
    ...
    extern bool Extract_ImageData_ROSFormat(const char *bag_infilepath, const std::string &img_topic, std::list<sensor_msgs::Image> &imgdatas)
    {
        ...
        foreach (rosbag::MessageInstance const m, view)
        {
            ...
            // if need, output cv::Mat to check the raw image
     ...
     * @note        1. The gnss solution data to read should be IPS pos format
     *              2. The gnss solution data is saved as RobotGVINS format defaultly
     *              3. If need, the GPS/Linux time will be converted
     *
    ...
    */
     ...
     * @note        1. The gnss raw to read should be Vision-RTK2 format
     *              2. The gnss observation and ephemris data is saved as RobotGVINS format defaultly
     *              3. The gnss raw data can be saved as binary ublox format (optional)
     *              4. If need, the GPS/Linux time will be converted
     *
     * @param[in]   char*       bag_infilepath           filepath to read
     * @param[in]   char*       gnssraw_outfilepath      filepath to write binary ublox raw data (optional)
    ...
    extern bool Extract_GNSSObsEphData_VisionRTK2Format(const char *bag_infilepath, const char *gnssraw_outfilepath, const std::string &gnssraw_topic, std::list<gnss_common::IPS_OBSDATA> &gnss_obsdata, std::list<gnss_common::IPS_GPSEPH> &gnss_ephdata)
    {
        ...
        foreach (rosbag::MessageInstance const m, view)
        {
            ...
            if (m.instantiate<datastreamio::VisionRTK2_GNSSRaw>() != nullptr)
            {
                // get the data pointer
                datastreamio::VisionRTK2_GNSSRaw::ConstPtr gnss_msg = m.instantiate<datastreamio::VisionRTK2_GNSSRaw>();
     ...
     * @note        1. The gnss epoch data to read should be Vision-RTK2 format
     *              2. The gnss epoch data is saved as RobotGVINS format defaultly
     *              3. If need, the GPS/Linux time will be converted
     *              4. FIXME: This function has some problems and needs to be modified
     *
     *
     * @param[in]   char*      bag_infilepath            filepath to read bag file
     * @param[in]   char*      gnssepoch_outfilepath     filepath to write data
    ...
    extern bool Extract_GNSSEpochData_VisionRTK2Format(const char *bag_infilepath, const char *gnssepoch_outfilepath, const std::string &gnssepoch_topic)
    {
        ...
        foreach (rosbag::MessageInstance const m, view)
        {
            if (m.instantiate<datastreamio::VisionRTK2_GNSSEpoch>() != nullptr)
            {
                // gnss epoch data
                datastreamio::VisionRTK2_GNSSEpoch::ConstPtr gnss_epoch = m.instantiate<datastreamio::VisionRTK2_GNSSEpoch>();
